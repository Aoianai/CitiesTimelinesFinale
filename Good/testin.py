...
class updateData():
    ...
    def parse_table(self):
        ...
        for row in data:
            ...
            self.increase_time = (self.base_time + 10)
            self.bu_id = row[0]
            ...
            if self.next_level_time == self.base_time and row[4] != '5':
                ...
                cursor.execute('''UPDATE coordb(SET b_level = ?
                                      SET next_level_time = ?
                                      WHERE b_id = ?
                                      AND next_level_time = ?)''', (self.increase_level, self.increase_time, self.bu_id, self.base_time))

                #cursor.execute('''UPDATE coordb SET (b_level, next_level_time) WHERE (b_id, next_level_time) values(?,?,?,?)''', (self.increase_level, self.increase_time, self.bu_id,self.base_time))
                ...
                if event.button == 1:                       ## and the button is the left mouse button
                    ...
                    while flag == 0:                 ## this loop will always run unless broken
                        ...
                        if image == image_lev_one:          ## if it is this image
                            ...
                            #next_level_time = str(next_level_time).zfill(10)
                        if image == image_lev_two:          ## repeat for all building levels
                            size = 40
                            level = 2
                            next_level_time = 15 + random.randrange(2,3) + base_time
                            #next_level_time = str(next_level_time).zfill(10)
                        if image == image_lev_three:
                            size = 60
                            level = 3
                            next_level_time = 25 + random.randrange(2,3) + base_time
                            #next_level_time = str(next_level_time).zfill(10)
                        if image == image_lev_four:
                            size = 80
                            level = 4
                            next_level_time = 35 + random.randrange(2,3) + base_time
                            #next_level_time = str(next_level_time).zfill(10)
                        if image == image_lev_five:
                            size = 100
                            level = 5
                            next_level_time = 99999 + base_time
                            #next_level_time = str(next_level_time).zfill(10)
